...
/*
 gcc -Wall wolfclient2.c -o ./wolfclient2 -lwolfssl
...
int main(int argc, char** argv)
{
    ...
    ret = wolfSSL_CTX_mcast_set_member_id(ctx, 23);
    ...
    ret = wolfSSL_mcast_peer_add(ssl, 0, 0);
    if (ret != SSL_SUCCESS) sys_err("mcast add peer 0 error");
    ret = wolfSSL_mcast_peer_add(ssl, 13, 0);
    if (ret != SSL_SUCCESS) sys_err("mcast add peer 13 error");
    ret = wolfSSL_mcast_peer_add(ssl, 255, 0);
